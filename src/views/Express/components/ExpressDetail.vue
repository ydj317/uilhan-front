<template>
    ...
    <div style="padding: 0 200px; height: calc(100vh - 100px) ;overflow-y: scroll; margin-right: 20px;">
		...
		<a-flex class="status-wrap fl-lb p30 mt25 top-border-000 bottom-border-f0f0f0 bg-fafafa">
			...
			<a-button class="fw fs14" @click="getHistory(state.bridgeOrderDetail['bridgeOrderId'])">배송 히스토리</a-button>
		</a-flex>
...
<!--					<div class="row-content m10">-->
						{{state.bridgeOrderDetail['ctrSeq'] === '인천1 해운' ? 'CJ대한통운' : 'CJ대한통운'}}
<!--					</div>-->
				</a-descriptions-item>
...
<!--		</a-flex>-->
	...
	>
		<template #title>
			<a-flex class="mt40">
				<span class="fs14 fw">배송 히스토리</span>
			</a-flex>
			<a-flex vertical class="info-wrap mt15 bor-bottom-0505050f top-border-000 ">
				<a-flex class="bottom-border-f0f0f0 h54" v-for="(item, index) in state.bridgeOrderStatusHistory" :key="index">
					<div class="bg-fafafa fw w20 fl-tc pl20">변경시간</div>
					<div class="w30 fl-tc pl20">{{item['insDate']}}</div>
					<div class="bg-fafafa fw w20 fl-tc pl20">주문상태</div>
					<div class="w25 fl-tc pl20">{{item['bridgeOrderStatus']}}</div>
				</a-flex>
			</a-flex>
		</template>
	</a-modal>
...
const getHistory = async (bridgeOrderId) => {
	await useBridgeApi().getBridgeOrderStatusHistory({bridgeOrderId}).then((res) => {
		if (res.status === "2000" && res.data) {
			state.bridgeOrderStatusHistory = res.data;
			console.log(state.bridgeOrderStatusHistory)
		}
	});
