<template>
  ...
  <a-descriptions title="수령자 정보" bordered :column="2" class="mt40" :labelStyle="{ width: '220px' }"
    ...
    <a-descriptions-item label="수령자 연락처 1">{{ orderData.items[0].itemOrgData.productOrder.shippingAddress.tel1
    }}</a-descriptions-item>
    <a-descriptions-item label="수령자 연락처 2">{{ orderData.items[0].itemOrgData.productOrder.shippingAddress.tel2
    }}</a-descriptions-item>
    <a-descriptions-item label="우편번호">{{
      orderData.items[0].itemOrgData.productOrder.shippingAddress.zipCode }}</a-descriptions-item>
    <a-descriptions-item label="국가">{{ orderData.items[0].itemOrgData.productOrder.shippingAddress.country }}
    </a-descriptions-item>
    <a-descriptions-item label="도시">{{ orderData.items[0].itemOrgData.productOrder.shippingAddress.city }}
    </a-descriptions-item>
    <a-descriptions-item label="주(state)">{{ orderData.items[0].itemOrgData.productOrder.shippingAddress.state }}
    </a-descriptions-item>
    <a-descriptions-item label="기본 주소">{{
      orderData.items[0].itemOrgData.productOrder.shippingAddress.baseAddress }}</a-descriptions-item>
    <a-descriptions-item label="상세 주소">{{ orderData.items[0].itemOrgData.productOrder.shippingAddress.detailedAddress }}
    </a-descriptions-item>
    ...
    <a-collapse v-model:activeKey="state.collapseKey" style="background-color: white;" collapsible="header">
      <a-collapse-panel :key="index" v-for="(item, index) in orderData.items">
        ...
        <a-descriptions title="상품정보" bordered :column="2" :labelStyle="{ width: '220px' }"
          ...
          <a-descriptions-item label="판매자 부담 할인액">{{ item.itemOrgData.productOrder.sellerBurdenDiscountAmount
          }}</a-descriptions-item>
          <a-descriptions-item label="상품 주문 금액(할인 적용 전 금액)">{{ item.itemOrgData.productOrder.totalProductAmount
          }}</a-descriptions-item>
          <a-descriptions-item label="총 결제 금액(할인 적용 후 금액)">{{ item.itemOrgData.productOrder.totalPaymentAmount
          }}</a-descriptions-item>
        ...
        <a-descriptions title="배송정보" bordered :column="2" class="mt40" :labelStyle="{ width: '220px' }"
          ...
          <a-descriptions-item label="배송비 최종 할인액">{{ item.itemOrgData.productOrder.deliveryDiscountAmount
          }}</a-descriptions-item>
          <a-descriptions-item label="배송비 합계">{{ item.itemOrgData.productOrder.deliveryFeeAmount }}</a-descriptions-item>
          <a-descriptions-item label="지역별 추가 배송비">{{ item.itemOrgData.productOrder.sectionDeliveryFee
          }}</a-descriptions-item>
          <a-descriptions-item label="배송비 형태(선불/착불/무료)">{{ item.itemOrgData.productOrder.shippingFeeType
          }}</a-descriptions-item>
          <a-descriptions-item label="배송비 정책(조건별 무료 등)">{{ item.itemOrgData.productOrder.deliveryPolicyType
          }}</a-descriptions-item>
        ...
        <a-descriptions title="해외배송정보" bordered :column="2" class="mt40" :labelStyle="{ width: '220px' }"
          :contentStyle="{ width: '500px' }">
          <a-descriptions-item label="구매자 개인통관고유부호">{{ item.itemOrgData.productOrder.individualCustomUniqueCode
          }}</a-descriptions-item>
        </a-descriptions>
