<template>
  ...
  <a-descriptions title="결제정보" bordered :column="2" class="mt40" :labelStyle="{ width: '220px' }" :contentStyle="{ width: '500px' }">
    <a-descriptions-item label="결제일시">{{ moment(orderData.orgData.paidAt).format('YYYY-MM-DD') }}</a-descriptions-item>
    <a-descriptions-item label="주문 할인액">{{ orderData.itemOrgData.discountPrice }}</a-descriptions-item>
    <a-descriptions-item label="결제 위치 구분">{{ orderData.orgData.refer }}</a-descriptions-item>
  </a-descriptions>
  ...
  <a-descriptions title="주문자 정보" bordered :column="2" class="mt40" :labelStyle="{ width: '220px' }" :contentStyle="{ width: '500px' }">
    ...
    <a-descriptions-item label="주문자 번호">{{ orderData.orgData.orderer.safeNumber }}</a-descriptions-item>
    <a-descriptions-item label="주문자 연락처">{{ orderData.orgData.orderer.ordererNumber }}</a-descriptions-item>
  </a-descriptions>

  <a-descriptions title="수령자 정보" bordered :column="2" class="mt40" :labelStyle="{ width: '220px' }"
                  :contentStyle="{ width: '500px' }">
    <a-descriptions-item label="수령자">{{ orderData.orgData.receiver.name }}</a-descriptions-item>
    <a-descriptions-item label="수령자 연락처 1">{{ orderData.orgData.receiver.safeNumber }}</a-descriptions-item>
    <a-descriptions-item label="수령자 연락처 2">{{ orderData.orgData.receiver.receiverNumber }}</a-descriptions-item>
    <a-descriptions-item label="우편번호">{{ orderData.orgData.receiver.postCode }}</a-descriptions-item>
    <a-descriptions-item label="기본 주소">{{ orderData.orgData.receiver.addr1 }}</a-descriptions-item>
    <a-descriptions-item label="상세 주소">{{ orderData.orgData.receiver.addr2 }}
    </a-descriptions-item>

    <a-descriptions-item label="개인통관번호">{{ orderData.orgData.overseaShippingInfoDto.personalCustomsClearanceCode }}
    </a-descriptions-item>
  </a-descriptions>
  ...
  <a-descriptions title="배송 정보" bordered :column="2" class="mt40" :labelStyle="{ width: '220px' }"
    ...
    <a-descriptions-item label="배송메모" >{{ orderData.orgData.parcelPrintMessage }}</a-descriptions-item>
  </a-descriptions>
</template>
