<template>
  <a-form ref="marketFormRef" :model="state.formData" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }"
    ...
    <a-form-item name="prd_update_client_id" @keyup="handleResetSyncStatus" label="상품수정 인증키"
      ...
      <a-input v-model:value="state.formData.prd_update_client_id" />
    </a-form-item>
    ...
    <a-form-item name="prd_update_client_secret" @keyup="handleResetSyncStatus" label="상품수정 비밀키"
      ...
      <a-input v-model:value="state.formData.prd_update_client_secret" />
    </a-form-item>

    <!-- 상품정보조회 -->
    <a-form-item name="prd_info_client_id" @keyup="handleResetSyncStatus" label="상품정보조회 인증키"
                 :rules="[{ required: true, message: '상품정보조회 인증키를 입력해 주세요.' }]">
      <a-input v-model:value="state.formData.prd_info_client_id" />
    </a-form-item>

    <a-form-item name="prd_info_client_secret" @keyup="handleResetSyncStatus" label="상품정보조회 비밀키"
                 :rules="[{ required: true, message: '상품정보조회 비밀키를 입력해 주세요.' }]">
      <a-input v-model:value="state.formData.prd_info_client_secret" />
    </a-form-item>

    <!-- 배송비정책조회 -->
    <a-form-item name="shipping_policy_client_id" @keyup="handleResetSyncStatus" label="배송비정책조회 인증키"
                 :rules="[{ required: true, message: '배송비정책조회 인증키를 입력해 주세요.' }]">
      <a-input v-model:value="state.formData.shipping_policy_client_id" />
    </a-form-item>

    <a-form-item name="shipping_policy_client_secret" @keyup="handleResetSyncStatus" label="배송비정책조회 비밀키"
                 :rules="[{ required: true, message: '배송비정책조회 비밀키를 입력해 주세요.' }]">
      <a-input v-model:value="state.formData.shipping_policy_client_secret" />
    </a-form-item>

    <!-- 기본전시조회 -->
    <a-form-item name="display_info_client_id" @keyup="handleResetSyncStatus" label="기본전시조회 인증키"
                 :rules="[{ required: true, message: '기본전시조회 인증키를 입력해 주세요.' }]">
      <a-input v-model:value="state.formData.display_info_client_id" />
    </a-form-item>

    <a-form-item name="display_info_client_secret" @keyup="handleResetSyncStatus" label="기본전시조회 비밀키"
                 :rules="[{ required: true, message: '기본전시조회 비밀키를 입력해 주세요.' }]">
      <a-input v-model:value="state.formData.display_info_client_secret" />
    </a-form-item>
              ...
              <a-form-item name="outbound_address_code" label="출고지"
                ...
                <a-select v-model:value="state.formData.outbound_address_code" placeholder="출고지를 선택해 주세요"
                  ...
                  <a-select-option :value="item.outbound_code"
                                   v-for="(item, key) in state.outboundAddressList" :key="key">{{ item.outbound_name
                    }}</a-select-option>
                </a-select>
              </a-form-item>
            </div>
