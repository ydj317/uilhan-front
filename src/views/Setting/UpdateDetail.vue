      ...
      <a-descriptions-item>
        <template #label>
          <a-space>
            ...
            <a-tooltip>
              <template #title>
                <div class="mb10">원가X환율 + (원가X환율)*(카드수수료+퍼센트수익)+최소 수익 + 해외배송비</div>
                <div>알리익스프레스의 경우 별도로 해외배송비용이 없습니다.</div>
              </template>
              <QuestionCircleOutlined/>
            </a-tooltip>
          </a-space>
...
function changeUseAi() {
  if (formState.userInfo.is_vip_time === false || formState.userInfo.use_ai_recommend === false) {
